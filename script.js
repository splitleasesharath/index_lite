...
// Setup Lottie Animations with Sequential Playback
function setupLottieAnimations() {
    // Wait a bit for Lottie to fully load
    setTimeout(() => {
        if (typeof lottie === 'undefined') {
            console.warn('Lottie library not loaded, retrying...');
            setTimeout(setupLottieAnimations, 500);
            return;
        }
    ...
    // Weeknight Animation
    animations.weeknight = lottie.loadAnimation({
        container: document.getElementById('weeknight-lottie-container'),
        renderer: 'svg',
        loop: false,
        autoplay: false,
        path: 'https://50bf0464e4735aabad1cc8848a0e8b8a.cdn.bubble.io/f1736800679546x885675666145660000/Days-of-the-week-lottie.json',
        rendererSettings: {
    ...
    const weeknightProgressFill = document.getElementById('weeknight-progress-fill');
    const weeknightPlayButton = document.getElementById('weeknight-play-button');

    if (weeknightPlayButton) {
        weeknightPlayButton.addEventListener('click', function () {
            // Stop other animations if playing
            if (currentlyPlaying && currentlyPlaying !== 'weeknight') {
                stopAnimation(currentlyPlaying);
            }
            currentlyPlaying = 'weeknight';
            
            weeknightPlayButton.style.display = 'none';
            animations.weeknight.goToAndPlay(0, true);
        });

        animations.weeknight.addEventListener('enterFrame', function() {
            const progress = (animations.weeknight.currentFrame / animations.weeknight.totalFrames) * 100;
            if (weeknightProgressFill) weeknightProgressFill.style.width = progress + '%';
        });

        animations.weeknight.addEventListener('complete', function () {
            weeknightPlayButton.style.display = 'flex';
            currentlyPlaying = null;
        });
    }

    // Weekend Animation
    animations.weekend = lottie.loadAnimation({
        container: document.getElementById('weekend-lottie-container'),
        renderer: 'svg',
        loop: false,
        autoplay: false,
        path: 'https://50bf0464e4735aabad1cc8848a0e8b8a.cdn.bubble.io/f1736800745354x526611430283845360/weekend-lottie%20%281%29.json',
        rendererSettings: {
            preserveAspectRatio: 'xMidYMid meet',
            className: 'lottie-svg'
        }
    ...
    const weekendProgressFill = document.getElementById('weekend-progress-fill');
    const weekendPlayButton = document.getElementById('weekend-play-button');

    if (weekendPlayButton) {
        weekendPlayButton.addEventListener('click', function () {
            // Stop other animations if playing
            if (currentlyPlaying && currentlyPlaying !== 'weekend') {
                stopAnimation(currentlyPlaying);
            }
            currentlyPlaying = 'weekend';
            
            weekendPlayButton.style.display = 'none';
            animations.weekend.goToAndPlay(0, true);
        });

        animations.weekend.addEventListener('enterFrame', function() {
            const progress = (animations.weekend.currentFrame / animations.weekend.totalFrames) * 100;
            if (weekendProgressFill) weekendProgressFill.style.width = progress + '%';
        });

        animations.weekend.addEventListener('complete', function () {
            weekendPlayButton.style.display = 'flex';
            currentlyPlaying = null;
        });
    }

    // Monthly Animation
    animations.monthly = lottie.loadAnimation({
        container: document.getElementById('monthly-lottie-container'),
        renderer: 'svg',
        loop: false,
        autoplay: false,
        path: 'https://50bf0464e4735aabad1cc8848a0e8b8a.cdn.bubble.io/f1736800780466x583314971697148400/Weeks-of-the-month-lottie.json',
        rendererSettings: {
            preserveAspectRatio: 'xMidYMid meet',
            className: 'lottie-svg'
        }
    ...
    const monthlyPlayButton = document.getElementById('monthly-play-button');

    if (monthlyPlayButton) {
        monthlyPlayButton.addEventListener('click', function () {
            // Stop other animations if playing
            if (currentlyPlaying && currentlyPlaying !== 'monthly') {
                stopAnimation(currentlyPlaying);
            }
            currentlyPlaying = 'monthly';
            
            monthlyPlayButton.style.display = 'none';
            animations.monthly.goToAndPlay(0, true);
        });

        animations.monthly.addEventListener('enterFrame', function() {
            const progress = (animations.monthly.currentFrame / animations.monthly.totalFrames) * 100;
            if (monthlyProgressFill) monthlyProgressFill.style.width = progress + '%';
        });

        animations.monthly.addEventListener('complete', function () {
            monthlyPlayButton.style.display = 'flex';
            currentlyPlaying = null;
        });
    }
    ...
    function stopAnimation(name) {
        if (animations[name]) {
            ...
            const button = document.getElementById(`${name}-play-button`);
